...
graph LR
    ...
    VIE["Vision Inference"]
    SRE["Speech Recognition"]
    CRE["Clinical Reasoning"]

    MVIE["Mock Vision Engine"]
    MSRE["Mock Speech Engine"]
    MCRE["Mock Clinical Engine"]
...
```mermaid
sequenceDiagram
    participant ASHA as Frontline Worker
    ...
    participant DOCTOR as PHC Doctor

    ASHA->>SENSOR: Capture screening data
    SENSOR->>AI: Send sensor data
    AI->>TRIAGE: Send structured findings
    TRIAGE->>TRIAGE: Evaluate clinical thresholds
    TRIAGE->>ASHA: Return triage recommendation
    ASHA->>DB: Save patient record
    ASHA->>DOCTOR: Wireless handover
```
